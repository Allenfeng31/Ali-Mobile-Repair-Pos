...
function getSupabaseClient() {
  ...
  if (!supabaseUrl || !supabaseKey) {
    throw new Error(
      'Missing server-side Supabase credentials. Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY before running the SEO worker.'
    );
  }
...
1. NO AI Buzzwords: Do NOT use high-frequency AI words: actually, additionally, align with, crucial, delve, emphasizing, enduring, enhance, fostering, garner, highlight (as verb), interplay, intricate/intricacies, key (as adjective), landscape (as abstract noun), pivotal, showcase, tapestry, testament, underscore, valuable, vibrant, ensures, delicate, challenging, precise, transparency, discuss whether, vital, allows us, understand that, keep in mind, instead of rushing, we focus on, focus on, focusing on, focused on, focuses on, inconvenience, spot hidden damage, prevent recurring issues, keep your data intact.
   ...
   - Replace "we understand that" or "understand that" or "inconvenience" with direct, simple statements (e.g. "Being without your phone is a hassle" instead of "We understand that being without... is a real inconvenience").
   - Replace "keep in mind" or "we keep data protection in mind" or "keep your data intact" with direct statements (e.g. "We do not erase your data").
   - Remove "instead of rushing through" or "we focus on... instead of rushing..." or "focusing on" entirely. Just write directly: "We follow these checks:" or "We test...".
   - Replace "spot hidden damage" or "prevent recurring issues" with specific descriptions of checkups (e.g. "We inspect the frame corners for bending" or "We check the display connectors for alignment").
...
10. Claim Safety and KB rules:
     - iPhone screen repairs:
       ...
       * Display warning messages: Successful data transfer usually resolves display warnings, but depends on whether the original hardware pairs correctly with the new panel. Do not guarantee message removal.
     ...
     - Samsung/Other screen repairs: Factory IP rating cannot be guaranteed after any phone has been opened. We rebuild the seal carefully, but IP rating is not guaranteed. Explicitly write "Factory IP rating cannot be guaranteed after any phone has been opened" rather than soft phrasing like "reduce its factory water resistance".
     - Diagnostics: Always include our No Fix, No Charge policy in the diagnostic section.
...
- Open with the customer's real-world symptom and daily-use frustration before explaining the technical repair. Do NOT write abstract introductory sentences like "A cracked screen is a common issue". Instead, start the first sentence of the article directly with a concrete, active customer symptom or situation: "Your OPPO screen cracked during a fall, or touch stopped working mid-call, and you are wondering how much it will cost to fix it."
...
- OPPO screen replacements: Since OPPO is not explicitly detailed in the master repair knowledge base, do NOT invent specific technical claims or custom opening procedures (like paired-chip calibration or specific seal properties). Instead, rely on our general display assembly taxonomy (digitizer glass, LCD or OLED panel, flex cables, and mid-frame). Ground the workbench-level diagnostic explanation in general display-assembly risks (e.g., protecting the display flex cables during opening, thorough adhesive cleanup from the mid-frame, and testing touch response post-repair). State honestly that because OPPO model ranges (like A, Reno, and Find) differ, we inspect the specific device model first and check stock before quoting. Write exactly: "Because OPPO's A, Reno, and Find ranges differ, we ask you to call with your model so we can confirm the right part and quote before you visit."
- Heading structure: Do not use rhetorical questions as headings (e.g. do not write "Why do screen replacement costs vary?"). Instead, use direct statements (e.g. "Screen replacement cost depends on display grade"). Every heading must be followed by substantial content, never a single short paragraph or single sentence.
- Direct Phrasing & No Hedging: Write direct, factual, confident statements instead of vague, soft, or defensive marketing language. 
  * Instead of "may cost less" write "costs less". Instead of "may encounter" write "you may see". Instead of "involve trade-offs in colour accuracy" write "trade touch sensitivity and durability for lower cost". Instead of "we are transparent that a professional repair is not the same as a brand-new factory-sealed unit" write "a professional repair is not factory-sealed".
  * Do NOT use cliché/awkward phrases: "takes the full impact" (write "hits the display first"), "leads to spider-webbed glass" (write "cracks the glass"), "we examine your device on the bench to confirm if a display replacement restores full function" (write "we check if a screen replacement will fix the issue"), or "water resistance is reduced once the phone is opened" (write "opening the phone reduces water resistance").
  * Do NOT hedge on repair times. Instead of saying "often finish within a few hours if parts are available", say: "If we have the display assembly in stock, screen replacement takes under an hour."
  * Do NOT use defensive/negative statements like "We do not erase or reset your device." Instead, state: "Your data remains safe on the device throughout the repair."
...
- Model-Specific Custom FAQs: Write custom FAQ questions specific to the exact device model and keyword, rather than generic warranty boilerplate. For example: "Will my screen repair remove the 'Genuine Apple Part' warning?", "Can you fix the screen if the frame is bent?", or "How long does the repair take if you have the part in stock?". Keep FAQs short, practical, and focused on real customer questions (such as part availability, booking requirements, or repair speed) rather than defensive disclaimer language.

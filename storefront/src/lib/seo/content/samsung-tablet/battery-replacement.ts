...
export function buildSamsungTabletBatteryReplacementPocket(
  ...
  const diagnosticSteps = [
    ...
    },
    {
      ...
      description: 'Once approved, we complete the battery repair using the path confirmed for that Galaxy Tab model.',
    },
    {
      step: '07',
      title: 'Perform post-repair function checks',
      description: `Before handover, we retest ${getSamsungTabletPostRepairChecks('battery-replacement')}.`,
    },
  ] as const;
  ...
  return {
    ...
    metaTitle: buildSamsungTabletMetaTitle(config, 'battery-replacement'),
    metaDescription: buildSamsungTabletMetaDescription(config, 'battery-replacement'),
    heroSubtitle: buildSamsungTabletHeroSubtitle(config, 'battery-replacement'),
    schemaDescription: buildSamsungTabletSchemaDescription(config, 'battery-replacement'),
    supportLabel: getSamsungTabletSupportLabel(config),
    quickAnswer: `${config.modelName} battery replacement helps when the tablet drains quickly, shuts down unexpectedly, shows unstable percentage behaviour, or no longer stays powered for normal use. At Ali Mobile & Repair we confirm the exact Galaxy Tab model and model code, inspect the reported power fault before the repair option is confirmed, and test the related functions after repair. Customers can book online or visit our Ringwood store for inspection.`,
    workbenchHeadings: {
    ...
    repairOptions: [
      ...
      },
      {
        ...
        shortDescription: `We confirm the exact ${config.modelName} model code so the battery repair information matches the correct Galaxy Tab version.`,
      ...
      },
      {
        ...
        shortDescription: `After repair we retest ${getSamsungTabletPostRepairChecks('battery-replacement')} before handover.`,
    ...
    diagnosticProcess: buildSamsungTabletDiagnosticProcessSection(config, 'battery-replacement', diagnosticSteps),
    serviceSection: buildSamsungTabletServiceSection(config, 'battery-replacement'),
    localService: buildSamsungTabletLocalServiceSection(config, 'battery-replacement'),
    finalCta: buildSamsungTabletFinalCtaSection(config, 'battery-replacement'),
    faq: [
      {
      ...
      {
        question: `Can I book this Samsung Tablet battery repair online?`,

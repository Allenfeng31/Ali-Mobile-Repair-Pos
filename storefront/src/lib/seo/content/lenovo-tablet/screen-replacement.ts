...
export function buildLenovoTabletScreenReplacementPocket(
  ...
  const diagnosticSteps = [
    ...
    },
    {
      ...
      description: 'Once approved, we complete the screen repair using the repair path confirmed for that Lenovo tablet model.',
    },
    {
      step: '07',
  ...
  return {
    ...
    metaTitle: buildLenovoTabletMetaTitle(config, 'screen-replacement'),
    metaDescription: buildLenovoTabletMetaDescription(config, 'screen-replacement'),
    heroSubtitle: buildLenovoTabletHeroSubtitle(config, 'screen-replacement'),
    schemaDescription: buildLenovoTabletSchemaDescription(config, 'screen-replacement'),
    supportLabel: getLenovoTabletSupportLabel(config),
    quickAnswer: `${config.modelName} screen replacement helps with cracked glass, black display faults, touch issues, lines, flickering, and colour or brightness problems. At Ali Mobile & Repair we confirm the exact Lenovo tablet model and model code, inspect the fault before the repair option is confirmed, and test the related functions after repair. Customers can book online or visit our Ringwood store for inspection.`,
    workbenchHeadings: {
    ...
    repairOptions: [
      ...
      },
      {
        ...
        shortDescription: `We confirm the exact ${config.modelName} model code so the screen repair information matches the correct Lenovo tablet version.`,
    ...
    commonProblems: [
      ...
      { title: 'Touch not responding', description: 'A Lenovo tablet screen can still light up while the touch layer stops responding normally.' },
    ...
    diagnosticProcess: buildLenovoTabletDiagnosticProcessSection(config, 'screen-replacement', diagnosticSteps),
    serviceSection: buildLenovoTabletServiceSection(config, 'screen-replacement'),
    localService: buildLenovoTabletLocalServiceSection(config, 'screen-replacement'),
    finalCta: buildLenovoTabletFinalCtaSection(config, 'screen-replacement'),
    faq: [
      {
      ...
      },
      {
        ...
        answer: 'We confirm the exact Lenovo tablet model and model code, inspect the visible damage, test the display and touch response, and then explain the suitable repair option through the existing price or quote path.',
      },
      {
      ...
      {
        question: `Can I book this Lenovo Tablet screen repair online?`,

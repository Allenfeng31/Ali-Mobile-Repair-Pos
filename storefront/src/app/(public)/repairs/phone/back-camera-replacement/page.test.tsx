...
describe('back camera shared hierarchy route', () => {
  it('passes an exact selected device and canonical booking helper to the landing for a valid pair', async () => {
    ...
    expect(props.hierarchy).toMatchObject({
      ...
      selectedDevice: { selectedDevice: { brand: 'Samsung', model: 'Galaxy S21' }, booking: { href: '/book-repair?category=phone&service=Back+Camera+Replacement&brand=Samsung&model=Galaxy+S21&brandSlug=samsung&modelSlug=galaxy-s21&serviceSlug=back-camera-replacement' } },
    });
  });

  it('fails closed for model-only and service override query state without changing metadata', async () => {
    fetchRepairCatalogMock.mockResolvedValue(catalogue());
    for (const query of [{ model: 'galaxy-s21' }, { brand: 'samsung', model: 'galaxy-s21', service: 'Front Camera Replacement' }]) {

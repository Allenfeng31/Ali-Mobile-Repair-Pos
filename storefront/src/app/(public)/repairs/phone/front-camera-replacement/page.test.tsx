...
describe('front camera shared hierarchy route', () => {
  it('passes an exact selected device and canonical booking helper to the landing for a valid pair', async () => {
    ...
    expect(props.hierarchy).toMatchObject({
      ...
      selectedDevice: { selectedDevice: { brand: 'Huawei', model: 'P30' }, booking: { href: '/book-repair?category=phone&service=Front+Camera+Replacement&brand=Huawei&model=P30&brandSlug=huawei&modelSlug=p30&serviceSlug=front-camera-replacement' } },
    });
  });

  it('fails closed for model-only and service override query state without changing metadata', async () => {
    fetchRepairCatalogMock.mockResolvedValue(catalogue());
    for (const query of [{ model: 'p30' }, { brand: 'huawei', model: 'p30', service: 'Back Camera Replacement' }, { brand: 'samsung', model: 'galaxy-s21' }]) {

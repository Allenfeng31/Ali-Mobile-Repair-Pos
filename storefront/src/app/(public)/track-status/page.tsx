...
export default function TrackStatusPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center py-20 px-4 w-full bg-slate-50">
      
      <div className="text-center mb-8 mt-16">
        <h1 className="text-2xl font-bold text-slate-800">Track Repair Status</h1>
        <p className="text-gray-500 mt-2">Enter your Order ID (found on your receipt) to check the real-time status of your device repair.</p>
      </div>

      <div className="w-full max-w-md bg-white rounded-2xl shadow-sm p-8 flex flex-col items-center text-center">
        {!repair ? (
          <form onSubmit={performCheck} className="w-full flex flex-col gap-4">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="w-full bg-[#007aff] text-white rounded-lg p-3 font-semibold hover:bg-blue-600 transition-colors disabled:opacity-50 mt-2"
            >

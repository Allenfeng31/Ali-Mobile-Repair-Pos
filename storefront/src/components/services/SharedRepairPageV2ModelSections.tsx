        ...
        {supportedModels.map((model) => {
          ...
          return (
            <article id={model.modelSlug} key={model.modelSlug} className="scroll-mt-28 flex flex-col items-center text-center rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm shadow-blue-950/5">
              ...
              <p className="mt-4 text-2xl font-black text-blue-700">{pricingStrategy?.mode === 'fixed' ? `$${pricingStrategy.fixedPrice}` : priceCandidate ? getSharedRepairCandidatePriceLabel(priceCandidate) : 'Quote on Request'}</p>

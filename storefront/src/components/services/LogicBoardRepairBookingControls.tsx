...
type LogicBoardRepairBookingControlsProps = Readonly<{
  basePath: string;
...
export default function LogicBoardRepairBookingControls({
  basePath,
  candidates,
}: LogicBoardRepairBookingControlsProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const bookingService = 'Logic Board Repair';
  ...
  return (
    <div className="flex w-full max-w-md flex-col items-center gap-4">
      <label className="w-full text-left text-sm font-bold text-slate-800" htmlFor="logic-board-model">
        Choose your phone model (optional)
        <select
          id="logic-board-model"

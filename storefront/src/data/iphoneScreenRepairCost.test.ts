...
describe("iPhone screen repair cost article data", () => {
  it("keeps the supplied 44-model table exact, including only the approved price-cell updates", () => {
  ...
  it("loads the article through the existing markdown blog source with truthful dates", async () => {
    ...
    expect(post.updated_at).toBe("2026-09-07");
    expect(IPHONE_SCREEN_REPAIR_COST_STATIC_METADATA).toEqual({

  ...
  return (
    <div className="space-y-10">
      ...
      <div className="mb-10 text-left">
        <h2 className="text-3xl font-extrabold text-blue-500 tracking-tight mb-2">System Settings</h2>
        <p className="text-zinc-400 font-body">Manage your hardware connections and document preferences.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        <div className="md:col-span-5 flex flex-col gap-8">
          ...
          <section className="bg-zinc-900/50 rounded-3xl p-8 border border-zinc-800/5">
            <h3 className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em] mb-6">Employee Information</h3>
            
            <div className="space-y-6">
              <div className="flex gap-4 items-center mb-2">
                <div className="w-16 h-16 rounded-2xl bg-blue-500/10 flex items-center justify-center text-blue-500 font-black text-2xl border border-blue-500/20 shadow-inner uppercase">
                  {(currentUser?.username || 'A').charAt(0)}
                </div>
                <div>
                  <p className="text-lg font-black text-white leading-tight">{currentUser?.username || 'Employee'}</p>
                  <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">{currentUser?.role || 'Staff'}</p>
                </div>
              </div>

              <div className="space-y-4 pt-2">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest pl-1">Update Name</label>
                  <input 
                    type="text" 
                    value={newName} 
                    onChange={(e) => setNewName(e.target.value)}
                    placeholder="Enter new display name..."
                    className="w-full bg-black border border-zinc-800/10 rounded-2xl px-4 py-3 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest pl-1">Change Password</label>
                  <input 
                    type="password" 
                    value={newPassword}
                ...
                <button 
                  ...
                  className="w-full bg-blue-500 text-white py-3.5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-blue-500/20 hover:opacity-95 transition-all active:scale-[0.98] flex items-center justify-center gap-2"
                >
              ...
              <div className="flex items-center justify-between bg-black p-5 rounded-2xl border border-zinc-800/10 shadow-sm opacity-60">
                <div className="flex items-center gap-4">
                  <div className="p-3 bg-red-500/10 rounded-2xl text-red-500">
                    <LogOut size={22} />
                  </div>
                  <div>
                    <p className="text-sm font-black text-white uppercase tracking-wider">End Session</p>
                    <p className="text-[10px] font-bold text-zinc-400 mt-0.5">Logout for security</p>
                  </div>
                </div>
                <button 
                  onClick={onLogout}
                  className="bg-zinc-800 text-red-500 px-4 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-red-500/20 hover:bg-red-500 hover:text-white transition-all"
                >
                  Sign Out
                </button>
              </div>
          ...
          <section className="bg-zinc-900/50 rounded-3xl p-8 border border-zinc-800/5">
            <div className="flex items-center gap-3 mb-6">
              <div className={cn(
                ...
                pushEnabled ? "bg-emerald-50 text-emerald-600" : "bg-zinc-800 text-zinc-400"
              )}>
                {pushEnabled ? <BellRing size={22} /> : <Bell size={22} />}
              </div>
              <div>
                <h3 className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em]">Push Notifications</h3>
                <p className="text-[10px] text-zinc-400/60 mt-0.5">
            ...
            <div className="space-y-4">
              {/* Status indicator */}
              <div className="flex items-center gap-3 p-4 rounded-2xl bg-zinc-800 border border-zinc-800/10">
                <div className={cn(
                  "w-3 h-3 rounded-full flex-shrink-0 transition-colors",
                  pushEnabled ? "bg-emerald-500 shadow-lg shadow-emerald-500/50 animate-pulse" : "bg-white-variant/20"
                )} />
                <span className={cn(
                  "text-sm font-bold",
                  pushEnabled ? "text-emerald-500" : "text-zinc-400"
                )}>
              ...
              <button 
                ...
                className={cn(
                  ...
                  pushEnabled 
                    ? "bg-zinc-800 text-zinc-400 border border-zinc-800/10 hover:bg-red-500/10 hover:text-red-500 hover:border-red-500/20 shadow-none" 
              ...
              {pushEnabled && (
                <button 
                  ...
                  className={cn(
                    "w-full py-3 rounded-2xl font-bold text-[10px] uppercase tracking-widest transition-all flex items-center justify-center gap-2",
                    testPushSent 
                      ? "bg-emerald-50 text-emerald-600 border border-emerald-200"
                      : "bg-zinc-800 text-zinc-400 border border-zinc-800/10 hover:text-blue-500 hover:border-blue-500/20"
                  )}
                >
          ...
          {/* SMS Alerts Toggle */}
          <section id="sms-alerts-section" className="bg-zinc-900/50 rounded-3xl p-8 border border-zinc-800/5">
            <div className="flex items-center gap-3 mb-6">
              <div className={cn(
                "p-2 rounded-2xl transition-colors",
                smsAlertsEnabled ? "bg-amber-50 text-amber-600" : "bg-zinc-800 text-zinc-400"
              )}>
                {smsAlertsEnabled ? <SmartphoneNfc size={22} /> : <Smartphone size={22} />}
              </div>
              <div>
                <h3 className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em]">SMS Alerts</h3>
                <p className="text-[10px] text-zinc-400/60 mt-0.5">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 p-4 rounded-2xl bg-zinc-800 border border-zinc-800/10">
                <div className={cn(
                  "w-3 h-3 rounded-full flex-shrink-0 transition-colors",
                  smsAlertsEnabled ? "bg-amber-500 shadow-lg shadow-amber-500/50 animate-pulse" : "bg-white-variant/20"
                )} />
                <span className={cn(
                  "text-sm font-bold",
                  smsAlertsEnabled ? "text-amber-500" : "text-zinc-400"
                )}>
              ...
              {/* Cost-saving info */}
              <p className="text-[10px] text-zinc-400/60 leading-relaxed px-1">
          ...
          <section id="google-sync-section" className="bg-zinc-900/50 rounded-3xl p-8 border border-zinc-800/5">
            <div className="flex items-center gap-3 mb-6">
              <div className={cn(
                "p-2 rounded-2xl transition-colors",
                googleSyncEnabled ? "bg-blue-50 text-blue-600" : "bg-zinc-800 text-zinc-400"
              )}>
                <Users size={22} />
              </div>
              <div>
                <h3 className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em]">Google Contacts Sync</h3>
                <p className="text-[10px] text-zinc-400/60 mt-0.5">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 p-4 rounded-2xl bg-zinc-800 border border-zinc-800/10">
                <div className={cn(
                  "w-3 h-3 rounded-full flex-shrink-0 transition-colors",
                  googleSyncEnabled ? "bg-blue-500 shadow-lg shadow-blue-500/50 animate-pulse" : "bg-white-variant/20"
                )} />
                <span className={cn(
                  "text-sm font-bold",
                  googleSyncEnabled ? "text-blue-500" : "text-zinc-400"
                )}>
        ...
        <div className="md:col-span-7">
          <section className="bg-zinc-900/50 rounded-[2rem] p-8 h-full flex flex-col border border-zinc-800/5">
            <div className="flex items-center gap-3 mb-10">
              <Receipt className="text-blue-500" size={24} />
              <h3 className="text-xl font-black text-blue-500 tracking-tight">Invoice Templates</h3>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 flex-grow">
              <div className="space-y-8">
                <div className="space-y-3">
                  <label className="block text-[10px] font-black text-zinc-400 uppercase tracking-[0.15em]">Invoice Header</label>
                  <textarea 
                    className="w-full bg-zinc-800 border-none rounded-2xl text-sm p-5 focus:ring-2 focus:ring-blue-500/20 focus:bg-zinc-800 transition-all resize-none font-medium h-32" 
                    value={header}
                    onChange={(e) => setHeader(e.target.value)}
                  />
                </div>
                <div className="space-y-3">
                  <label className="block text-[10px] font-black text-zinc-400 uppercase tracking-[0.15em]">Footer Disclaimer</label>
                  <textarea 
                    className="w-full bg-zinc-800 border-none rounded-2xl text-sm p-5 focus:ring-2 focus:ring-blue-500/20 focus:bg-zinc-800 transition-all resize-none font-medium h-32" 
                    value={footer}
                ...
                <div className="flex gap-4 pt-4">
                  <button 
                    onClick={handleSave}
                    className="flex-1 bg-blue-500 hover:bg-blue-600 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-blue-500/20 hover:opacity-90 transition-all active:scale-[0.98]"
                  >
                    {isSaved ? <CheckCircle2 size={16} /> : <Save size={16} />}
                    {isSaved ? 'SAVED' : 'SAVE CHANGES'}
                  </button>
                  <button className="bg-zinc-800 text-white px-6 rounded-2xl font-black hover:bg-zinc-700-fixed transition-all">
                    <Eye size={20} />
                  </button>
                </div>
              </div>
                ...
                <div className="relative bg-white/90 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-white/40 h-full flex flex-col">
                  ...
                  <div className="flex-grow space-y-4">
                    ...
                    <div className="pt-4 flex justify-between">
                      ...
                      <span className="text-[11px] font-black text-blue-500">$254.00</span>
                    </div>
                  </div>
          ...
          {isSuperAdmin && (
            <section className="bg-zinc-900/50 rounded-[2rem] p-8 border border-zinc-800/5 mt-8">
              <div className="flex items-center gap-3 mb-8">
                <div className="p-2 bg-amber-50 rounded-2xl text-amber-600">
                  <Layers size={24} />
                </div>
                <div>
                  <h3 className="text-xl font-black text-blue-500 tracking-tight">Quality Tiers Management</h3>
                  <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mt-1">Super Admin Dictionary</p>
                </div>
              </div>

              <div className="space-y-6">
                <div className="bg-zinc-800 p-5 rounded-2xl border border-zinc-800/10 space-y-4">
                  <h4 className="text-xs font-black text-white uppercase tracking-wider">Add New Tier</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <input 
                      ...
                      className="w-full bg-black border border-zinc-800/10 rounded-2xl px-4 py-3 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-amber-500/20 transition-all"
                    />
                    <input 
                      type="text" 
                      value={newTierDesc}
                      ...
                      className="md:col-span-2 w-full bg-black border border-zinc-800/10 rounded-2xl px-4 py-3 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-amber-500/20 transition-all"
                    />
                  </div>
                  <button 
                    onClick={handleCreateTier}
                    disabled={!newTierName || !newTierDesc}
                    className="bg-amber-500 text-white px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-amber-600 transition-all disabled:opacity-50"
                  >
                    Create Tier
                  </button>
                </div>
                ...
                <div className="space-y-3">
                  {qualityTiers.map(tier => (
                    <div key={tier.id} className="bg-black p-4 rounded-2xl border border-zinc-800/5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div className="flex-1">
                        ...
                        {editingTierId === tier.id ? (
                          <input 
                            ...
                            className="w-full bg-zinc-800 border border-zinc-800/10 rounded-2xl px-3 py-2 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-amber-500/20"
                          />
                        ) : (
                        ...
                        {editingTierId === tier.id ? (
                          <>
                            <button 
                              ...
                              className="p-2 bg-emerald-50 text-emerald-600 rounded-2xl hover:bg-emerald-100 transition-colors"
                            >
                              <CheckCircle2 size={16} />
                            </button>
                            <button 
                              onClick={() => { setEditingTierId(null); setEditingTierDesc(''); }}
                              className="p-2 bg-red-500/5 text-red-500 rounded-2xl hover:bg-red-500/10 transition-colors"
                            >
                              <Trash2 size={16} />
                            </button>
                          </>
                          ...
                          <>
                            <button 
                              ...
                              className="p-2 bg-zinc-800 text-white rounded-2xl hover:opacity-80 transition-opacity"
                            >
                              <Edit2 size={16} />
                            </button>
                            <button 
          ...
          <section className="bg-zinc-900/50 rounded-[2rem] p-8 border border-zinc-800/5 mt-8">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h3 className="text-xl font-black text-blue-500 tracking-tight">Quick SMS Generator</h3>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-3">
                  ...
                  <input 
                    ...
                    className="w-full bg-black border border-zinc-800/10 rounded-2xl px-5 py-4 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all"
                  />
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest pl-1">Repair</label>
                  <input 
                    type="text" 
                    value={smsRepair}
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest pl-1">Price (Amount)</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    placeholder="200"
                    className="w-full bg-black border border-zinc-800/10 rounded-2xl pl-9 pr-5 py-4 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all"
                  />
                </div>
              </div>
              ...
              {(smsModel || smsRepair || smsAmount) && (
                <div className="mt-6 p-4 bg-zinc-800 rounded-2xl border border-zinc-800/10 text-sm whitespace-pre-wrap font-medium text-zinc-400">
      ...
      {!isSuperAdmin && (
        <details className="mt-4 opacity-30 hover:opacity-100 transition-opacity text-[10px] text-zinc-400">
          <summary className="cursor-pointer font-mono">🔍 Auth Debug (dev only)</summary>
          <pre className="mt-2 p-3 bg-zinc-800 rounded-2xl overflow-auto max-h-40 font-mono text-[9px] leading-tight">

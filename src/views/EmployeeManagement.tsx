    ...
    <div className="h-full flex flex-col max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-6">
          <button 
            onClick={onBack}
            className="w-12 h-12 bg-zinc-900-high rounded-2xl flex items-center justify-center text-zinc-400 hover:text-blue-500 hover:bg-blue-500/10 transition-all"
          >
            <ArrowLeft size={24} />
          </button>
          <div>
            <h1 className="text-4xl font-black text-white tracking-tighter">Employee <span className="text-blue-500 italic">Management</span></h1>
            <p className="text-zinc-400 font-medium">Control platform-wide access and create secure staff accounts.</p>
          </div>
        </div>
        
        <button 
          onClick={() => setShowAddModal(true)}
          className="flex items-center gap-2 px-6 py-4 bg-blue-500 text-on-primary rounded-3xl font-black text-sm uppercase tracking-widest shadow-xl shadow-blue-500/20 hover:scale-105 active:scale-95 transition-all"
        >
          <UserPlus size={18} />
          New Employee
        </button>
      ...
      <div className="flex-1 overflow-y-auto space-y-6 pb-12">
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-pulse">
            {[1, 2, 3].map(i => (
              <div key={i} className="h-64 bg-zinc-900 rounded-[2.5rem]" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {employees.map((emp) => (
              <motion.div
                ...
                className="bg-zinc-900/50 border border-zinc-800/10 rounded-[2.5rem] p-8 flex flex-col shadow-sm hover:shadow-xl transition-all"
              >
                <div className="flex items-start justify-between mb-6">
                  <div className="flex items-center gap-4">
                    <div className="w-14 h-14 bg-blue-500/10 rounded-2xl flex items-center justify-center text-blue-500 font-black text-xl border border-blue-500/20">
                ...
                <div className="space-y-4 flex-1">
                        ...
                        <button
                          ...
                          className={`flex items-center gap-2 p-3 rounded-2xl border transition-all ${
                            isActive 
                              ...
                              : 'bg-zinc-900-high border-transparent text-zinc-400'
                          }`}
                        >
                          <perm.icon size={14} className={isActive ? perm.color : 'text-zinc-400/40'} />
                  ...
                  {emp.can_give_discount && (
                    <div className="pt-2 animate-in fade-in slide-in-from-top-2">
                      <label className="block text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-2 pl-1">
                        Max Discount Limit ($)
                      </label>
                      <input 
                        type="number"
                        ...
                        className="w-full bg-zinc-900-high rounded-2xl px-4 py-2.5 text-xs font-bold text-white outline-none focus:ring-2 focus:ring-blue-500/20 transition-all"
                      />
                    </div>
                  )}
                </div>

                <div className="mt-8 pt-6 border-t border-zinc-800/10 flex justify-between items-center text-[10px] font-bold text-zinc-400 uppercase tracking-widest">
        ...
        {showAddModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-2xl bg-zinc-900/50 rounded-[3rem] border border-zinc-800/20 shadow-2xl p-10 overflow-hidden"
            >
              <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                  <h2 className="text-3xl font-black text-white tracking-tight">Onboard Staff</h2>
                  <p className="text-zinc-400 font-medium">Create a secure Supabase Auth account.</p>
                </div>
                <button 
                  onClick={() => setShowAddModal(false)}
                  className="ml-auto w-12 h-12 rounded-full hover:bg-zinc-900-high flex items-center justify-center text-zinc-400"
                >
                  <ArrowLeft size={24} />
                </button>
              </div>
              ...
              <form onSubmit={handleCreateEmployee} className="space-y-8">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <Mail size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400/40" />
                      <input 
                        type="text"
                        value={newEmail}
                        ...
                        className="w-full bg-zinc-900-high rounded-2xl pl-12 pr-4 py-4 text-sm font-bold text-white outline-none focus:ring-2 focus:ring-blue-500/20 transition-all placeholder:text-zinc-400/30"
                      />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest pl-1">Initial Password</label>
                    <div className="relative">
                      <Key size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400/40" />
                      <input 
                        type="password"
                        value={newPassword}
                ...
                <div className="space-y-4">
                  <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest pl-1">Initial Permissions</label>
                        ...
                        <button
                          ...
                          className={`flex items-center gap-3 p-4 rounded-2xl border transition-all ${
                            isActive 
                              ...
                              : 'bg-zinc-900-high border-transparent text-zinc-400 hover:bg-zinc-800'
                          }`}
                        >
                          <perm.icon size={16} className={isActive ? perm.color : 'text-zinc-400/40'} />
                          <span className="text-[10px] font-black uppercase tracking-tight">{perm.label}</span>
                        </button>
                      );
                    })}
                  ...
                  {newPermissions.can_give_discount && (
                     <div className="pt-2 animate-in fade-in slide-in-from-top-2">
                      ...
                      <input 
                        ...
                        className="w-full bg-zinc-900-high rounded-2xl px-4 py-3.5 text-sm font-bold text-white outline-none focus:ring-2 focus:ring-blue-500/20 transition-all"
                      />
                    </div>
                  )}
                </div>
                ...
                <button
                  ...
                  className="w-full py-5 bg-white text-surface rounded-3xl font-black uppercase tracking-widest shadow-2xl hover:brightness-110 active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-3"
                >
